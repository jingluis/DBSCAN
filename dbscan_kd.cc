...
struct Point{
	...
	int label;
	bool checking;
	Point(float xx, float yy, int ii, int ll = UNDEFINED, bool b = false){
		x = xx;
		y = yy;
		index = ii;
		label = ll;
...
void union_vector(pointIndexArr& s, const pointIndexArr& N){

	for(int i = 0; i < N.size(); ++i){
		if(not points[N[i].second].checking) {	
			points[N[i].second].checking = true;
			s.push_back(N[i]);	
		}	
	}
}
...
void DBSCAN(){
	for(int i = 0; i < points.size(); ++i){
		...
		pointIndexArr Neighbours;
		tree.neighborhood_without_itself(Neighbours, {points[i].x, points[i].y}, eps);
		if(Neighbours.size() + 1 < minpts){
			points[i].label = NOISE;
			continue;
		}
		for(auto w: Neighbours) points[w.second].checking = true;
		points[i].checking = true;
		++C;
		points[i].label = C - 1;
		pointIndexArr S = Neighbours;
		int n = S.size();
		for(int j = 0; j < n; ++j){
			if(points[S[j].second].label == NOISE){
				points[S[j].second].label = C - 1;	
			}
			if(points[S[j].second].label != UNDEFINED) continue;
			points[S[j].second].label = C - 1;
			tree.neighborhood(Neighbours, S[j].first, eps);
