...
void RangeQuery(Point p, vector <Point>& res, bool b){
	...
	if(b){
		for(int i = 0; i < points.size(); ++i){
			if(not(points[i].x == p.x and points[i].y == p.y) and check_distance(p,points[i])){
...
void union_vector(vector <Point>& s, const vector <Point>& N){
	...
	for(int i = 0; i < N.size(); ++i){
		if(not N[i].checking) {	
			points[N[i].index].checking = true;
			s.push_back(points[N[i].index]);	
		}	
	}
...
void DBSCAN(){
	for(int i = 0; i < points.size(); ++i){
		...
		for(int j = 0; j < n; ++j){
			S[j].checking = true;
			points[S[j].index].checking = true;
			if(S[j].label == NOISE){
				S[j].label = C - 1;
