...
void KDTree::neighborhood_(
    ...
    pointIndexArr nbh_s, nbh_o;
    if(b){
        if (d <= r2 and (point_t(*branch) != pt) ) {
    ...
    neighborhood_(nbh_s, section, pt, rad, (level + 1) % dim, b);
    nbh.insert(nbh.end(), nbh_s.begin(), nbh_s.end());
    if (dx2 < r2) {
        neighborhood_(nbh_o, other, pt, rad, (level + 1) % dim, b);
        nbh.insert(nbh.end(), nbh_o.begin(), nbh_o.end());
    }
};

void KDTree::neighborhood(
    pointIndexArr& nbh,  //
    const point_t &pt,               //
    const float &rad) {
    nbh.clear();
    size_t level = 0;
    neighborhood_(nbh, root, pt, rad, level);
}

void KDTree::neighborhood_without_itself(
    pointIndexArr& nbh,  //
    const point_t &pt,               //
    const float &rad) {
    nbh.clear();
    size_t level = 0;
    neighborhood_(nbh, root, pt, rad, level, true);
}
